# Ford Focus ST — Parts & Mods

> **Status:** `[ ]` want · `[~]` ordered · `[x]` installed

---

## Performance

### Engine / Intake
- [ ] [Cobb SF Intake](https://www.cobbtuning.com/products/ford-focus-st-short-ram-intake-system) — easy bolt-on, +8hp
- [ ] [Mountune High Flow Air Box](https://mountuneusa.com/collections/focus-st/products/high-flow-airbox) — OEM+ intake, warranty-friendly
- [ ] [K&N Drop-in Filter](https://www.knfilters.com) — BM5Z-9601-A size, direct swap
- [ ] [Mishimoto FMIC Kit](https://www.mishimoto.com/ford-focus-st-front-mount-intercooler-kit.html) — biggest bang for buck, reduces heat soak
- [ ] [Forge Motorsport Recirculation Valve](https://www.forge-motorsport.com) — OEM BOV replacement, better boost hold
- [ ] [Cobb Charge Pipe Kit](https://www.cobbtuning.com) — replaces OEM plastic boost pipes

### Exhaust
- [ ] [Borla ATAK Cat-Back](https://www.borla.com/ford-focus-st-catback-exhaust) — aggressive tone, polished tips
- [ ] [Mountune Cat-Back](https://mountuneusa.com/collections/focus-st) — refined, OEM quality
- [ ] [Milltek Non-Resonated Cat-Back](https://www.millteksport.com) — louder, more raspy
- [ ] [Cobb 3" Catted Downpipe](https://www.cobbtuning.com) — high-flow cat, keeps emissions compliance
- [ ] Resonator Delete Pipe — cheap first mod, opens up exhaust note

### Suspension
- [ ] [KW Variant 3 Coilovers](https://www.kwsuspension.com) — independently adjustable, best quality
- [ ] [Fortune Auto 500 Coilovers](https://www.fortuneauto.com) — great value, built to order
- [ ] [Eibach Pro-Kit Springs](https://www.eibach.com) — 25mm drop, retains OEM dampers
- [ ] [Whiteline 27mm Front Sway Bar](https://www.whiteline.com.au) — reduces understeer significantly
- [ ] [Whiteline 22mm Rear Sway Bar](https://www.whiteline.com.au) — pairs with front bar
- [ ] Whiteline End Links — required when upgrading sway bars
- [ ] Rear Trailing Arm Bushings — polyurethane, fixes rear camber slop
- [ ] Front Strut Tower Brace — reduces chassis flex

### Brakes
- [ ] [Hawk HPS Brake Pads — Front](https://www.hawkperformance.com) — HB432F.661, daily driver upgrade, low dust
- [ ] [Hawk HPS Brake Pads — Rear](https://www.hawkperformance.com) — HB533F.668
- [ ] [EBC Redstuff Pads — Front](https://www.ebcbrakes.com) — DP31573C, ceramic, very low dust
- [ ] [Stoptech Sport Drilled/Slotted Rotors — Front](https://www.stoptech.com) — 320mm
- [ ] [Stoptech Sport Drilled/Slotted Rotors — Rear](https://www.stoptech.com) — 302mm
- [ ] [Goodridge Stainless Brake Lines](https://www.goodridge.com) — sharper pedal feel
- [ ] [ATE Super DOT 4 Brake Fluid](https://www.ate-brakes.com) — higher dry boiling point than OEM spec

### Cooling
- [ ] [Mishimoto Aluminum Radiator](https://www.mishimoto.com/ford-focus-st-aluminum-radiator.html) — direct fit, better thermal capacity
- [ ] [Setrab Oil Cooler Kit](https://www.setrab.com) — important for track use, prevents oil breakdown
- [ ] [Forge Motorsport Oil Cooler](https://www.forge-motorsport.com) — FMOC001, bolt-on

### Drivetrain
- [ ] [Quaife ATB Differential](https://www.quaife.co.uk/products/ford-focus-st-atb-differential) — eliminates torque steer, single best handling mod ~$1,100
- [ ] [Cobb Short Throw Shifter](https://www.cobbtuning.com) — reduces throw by ~25%
- [ ] [Whiteline Shifter Bushing Kit](https://www.whiteline.com.au) — KIT008, removes shifter slop
- [ ] [Mountune Motor Mount Inserts](https://mountuneusa.com) — reduces engine movement, more direct feel
- [ ] [Exedy Stage 1 Clutch Kit](https://www.exedyclutch.com) — holds 350ft-lb, good daily option for power builds

### Tune / ECU
- [ ] [Cobb AccessPort V3](https://www.cobbtuning.com/products/ford-focus-st-accessport) — must-have, datalogging + maps, $649
- [ ] [Mountune MP215 Package](https://mountuneusa.com/collections/focus-st/products/mp215-performance-upgrade) — intake + tune + map, +28hp, OEM warranty-safe, ~$800
- [ ] [Mountune MP235 Package](https://mountuneusa.com/collections/focus-st/products/mp235-performance-upgrade) — FMIC + intake + tune, +37hp, ~$1,200
- [ ] EcoBoost Flex Fuel Kit — ethanol sensor + tune, requires AccessPort

---

## Exterior

### Wheels & Tires
- [ ] [Enkei RPF1 17x8 +45](https://www.enkei.com) — 225/45R17, 16.2lb, classic track wheel
- [ ] [Konig Hypergram 17x8 +45](https://www.konigwheels.com) — budget-friendly lightweight option
- [ ] [Michelin Pilot Sport 4S 235/40R18](https://www.michelin.com) — best daily/track tire in OE size
- [ ] [Continental ExtremeContact Sport 235/40R18](https://www.continentaltire.com) — great value performance tire
- [ ] [Falken Azenis RT660 235/40R18](https://www.falkentire.com) — track day / autocross tire
- [ ] Wheel Spacers 5x108 — 15mm front, 10mm rear for flush fitment

### Body / Aero
- [ ] [Mountune Front Splitter](https://mountuneusa.com) — subtle aero, OEM look
- [ ] Anderson Composites Carbon Fiber Hood — weight reduction, aggressive look
- [ ] OEM ST200 Side Skirts — JDM-spec side extensions
- [ ] [Rear Diffuser](https://www.ebay.com) — improves rear aero balance
- [ ] Focus RS Front Bumper Conversion — requires cutting, dramatic look change
- [ ] Tow Hook — both front and rear for track days

### Lighting
- [ ] [Morimoto XB LED Headlights](https://www.theretrofitsource.com) — plug and play, massive upgrade over OEM halogens
- [ ] [Diode Dynamics H16 LED Fog Lights](https://www.diodedynamics.com) — SL1 or SLF, direct fit
- [ ] [Diode Dynamics Interior LED Kit](https://www.diodedynamics.com) — full interior, warm white
- [ ] [Spyder LED Tail Lights](https://www.spyder-auto.com) — blacked out option
- [ ] Puddle Lights — Focus logo projection, door mirrors

### Wraps & Paint
- [ ] Roof Vinyl Wrap — satin black or gloss
- [ ] Mirror Cap Wraps — contrast color
- [ ] [Carbon Fiber Mirror Caps](https://www.ebay.com) — OEM replacement caps
- [ ] Hood Vinyl Wrap — racing stripe or full hood
- [ ] Plasti-Dip Grille Badge — blackout treatment

---

## Interior

### Seats / Harness
- [ ] [Recaro Sportster CS](https://www.recaro-automotive.com) — bolstered, OEM fitment brackets available
- [ ] [Corbeau GTS II Seats](https://www.corbeau.com) — more affordable, good support
- [ ] [Takata 4-Point Harness](https://www.takata.com) — 2" shoulder straps, requires harness bar
- [ ] Harness Bar — bolt-in 49" bar for rear seat delete / harness mounting
- [ ] Seat Heater Delete Switches — cleaner console if removing OEM heated seats

### Gauges & Displays
- [ ] [Cobb AccessPort V3 A-Pillar Mount](https://www.cobbtuning.com) — mounts AP where you can see it
- [ ] [AEM X-Series Boost/Vacuum Gauge](https://www.aemelectronics.com) — 30-0301, -30 to 35 psi
- [ ] [PLX DM-6 OBD2 Multi-Gauge](https://www.plxdevices.com) — shows boost, AFR, temps via OBD2
- [ ] [Innovate LC-2 Wideband AFR](https://www.innovatemotorsports.com) — proper wideband for tuning
- [ ] Dual A-Pillar Pod — custom fab or 3D printed for two gauges

### Audio
- [ ] [Pioneer DMH-W4660NEX](https://www.pioneerelectronics.com) — wireless CarPlay/Android Auto, floating 7"
- [ ] [Kenwood DMX7709S](https://www.kenwood.com) — wireless CarPlay, clean interface
- [ ] [Focal Integration IS 165TOE](https://www.focal.com) — OEM connector front speakers, no cutting
- [ ] [Dynamat Xtreme Door Kit](https://www.dynamat.com) — doors + trunk, huge audio quality improvement
- [ ] [Kicker CompR 10" Sub + Amp](https://www.kicker.com) — fits hatchback floor area

### Pedals & Steering
- [ ] [Mountune Aluminum Pedal Set](https://mountuneusa.com) — throttle, brake, clutch, direct fit
- [ ] [Agency Power Short Throw Shifter](https://www.agencypower.com) — AP-FFS-150
- [ ] [NRG Short Hub Adapter](https://www.nrginnovations.com) — SRK-170H for Focus
- [ ] [NRG Quick Release Gen 2.5](https://www.nrginnovations.com) — for aftermarket wheel
- [ ] [Momo Prototipo Steering Wheel](https://www.momo.it) — classic look, 350mm

### Other Interior
- [ ] [Weathertech Floor Mats](https://www.weathertech.com) — laser measured, full coverage
- [ ] [Cobb SF Shift Knob](https://www.cobbtuning.com) — weighted, better feel
- [ ] Window Tint — 35% front, 20% rear is popular combo
- [ ] Sound Deadening — Kilmat or Dynamat full car treatment
- [ ] [Footwell LED Lighting Kit](https://www.amazon.com) — ambient lighting strips

---

## Maintenance & Consumables

### Fluids & Filters
- [ ] Motorcraft AGSF32PM Spark Plugs — OEM spec, pre-gapped 0.028", replace every 30k mi (4 plugs)
- [ ] Motorcraft FL-910S Oil Filter — OEM spec
- [ ] Motorcraft 5W-30 Full Synthetic — 5.7qt capacity per change
- [ ] Ford BM5Z-9601-A OEM Air Filter — or K&N 33-2435 for reusable
- [ ] Ford DG9Z-19N619-A Cabin Air Filter — replace annually
- [ ] Motorcraft VC-3-B Orange Coolant — 50/50 premix, ~2.5qt top-up capacity
- [ ] DOT 4 LV Brake Fluid — Ford spec WSS-M6C65-A2, ATE or Motul RBF600
- [ ] Motorcraft Full Synthetic MTF — MT82 gearbox, 1.8L fill, reduces crunch in cold weather
- [ ] Motul 8100 X-clean+ 5W-30 — popular OEM+ oil upgrade

### Wear Items
- [ ] Front Brake Pads — 320mm front, replace around 30k mi depending on use
- [ ] Rear Brake Pads — 302mm rear
- [ ] Front Rotors — stock 320mm vented
- [ ] Rear Rotors — stock 302mm solid
- [ ] Drive Belt — Ford 6PK1763 serpentine, inspect at 60k
- [ ] Clutch Kit — stock clutch holds ~280ft-lb, upgrade if tuned
- [ ] OEM Wiper Blades — 26" driver / 16" passenger (Bosch Icon fit well)
- [ ] Thermostat — replace if running warm, common weak point on EcoBoost
- [ ] PCV Valve — inspect at 60k, clogging causes oil leaks
- [ ] Spark Plug Coil Pack Set — replace with plugs if misfiring at 60k+

### Tools
- [ ] Cobb AccessPort V3 — also functions as OBD2 scanner / datalLogger
- [ ] Autel MaxiLink ML619 — budget OBD2 scanner with ABS/SRS codes
- [ ] Oil Drain Pan — 8qt+ for 5.7qt capacity
- [ ] Torque Wrench — 0-150 ft-lb range, needed for wheel bolts (103 ft-lb) + drain plug (20 ft-lb)
- [ ] T30 Torx Bit Set — brake caliper slider pins
- [ ] 32mm Socket — front hub nut (one-time use if replacing wheel bearing)
- [ ] Oil Filter Wrench — fits FL-910S
- [ ] Jack Stands (x4) — never use just a floor jack

---

## Done

<!-- Move completed items here with [x] and note install date / cost -->
